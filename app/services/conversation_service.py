...
class ConversationService:
    ...
    async def get_user_conversations(self, user_id:str,limit:int,skip:int)->list[Conversationresponse]:
        logger.info("Listing conversations for user: %s (skip=%s, limit=%s)", user_id, skip, limit)
    ...
    async def get_coversation_detail(self, user_id:str,conv_id:str,limit:int)->CnoversationDetailRespone:
        logger.info("Fetching conversation detail for conv_id=%s user=%s", conv_id, user_id)
        conv = await self.conv_repo.get_conv_by_id(conv_id=conv_id)
        if not conv or conv.user_id != user_id:
            logger.warning("Conversation not found or access denied conv_id=%s user=%s", conv_id, user_id)
            return "error"
    ...
    async def delete_conversation(self, user_id:str,conv_id:str)->None:
        ...
        conv = await self.conv_repo.get_conv_by_id(conv_id=conv_id)
        if not conv or conv.user_id != user_id:
            logger.warning("Delete failed: conversation not found or access denied conv_id=%s user=%s", conv_id, user_id)
            return "error"
